...
const NaverShopping = () => {
  ...
  const dahlia = ["달리아(핑크-노랑)", "달리아(오렌지)"];
  ...
  const handleNaverSearch = async (flowername) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8000/naver-shopping?flowername=${search}`
      );
      //const response = await axios.get(`https://fdy-5349cb70f365.herokuapp.com/naver-shopping?flowername=${key}`);
      const jsonData = response.data;
      const filteredResults = jsonData.items.filter(
        (item) =>
          item.category3 === "씨앗/모종" || item.category3 === "조경수/묘목"
      );
  ...
  const card = (item) => (
    <React.Fragment>
      ...
      <CardContent>
        <Typography
          sx={{ fontSize: 18, wordBreak: "keep-all" }}
          color="black"
          gutterBottom
        >
        ...
        <Typography variant="body2">{item.mallName}</Typography>
      </CardContent>
      <CardActions>
        <Button
          size="small"
            ...
            <Grid container spacing={2} justifyContent="center">
              {naverShoppingResults.map((item, index) => (
                <Grid item key={index}>
                  <Card
                    variant="outlined"
                    sx={{ width: 300, height: "auto", margin: "0.5rem" }}
                  >
                    {card(item)}
                  </Card>
                </Grid>
