...
const NaverShopping = () => {
  ...
  const { key } = useParams();
  const noResult = ['꽃도라지', '나무양귀비', '비단목메꽃', '아카울리스용담', '왕자의 깃털', '카우틀레이아 스피카타', '카틀레야 라비아타', '달난초', '홍학꽃'];
  ...
  const handleNaverSearch = async (flowername) => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/naver-shopping?flowername=${search}`);
      //const response = await axios.get(`https://fdy-5349cb70f365.herokuapp.com/naver-shopping?flowername=${key}`);
      const jsonData = response.data;
      const filteredResults = jsonData.items.filter((item) => (item.category3 === "씨앗/모종" || item.category3 === "조경수/묘목") );
