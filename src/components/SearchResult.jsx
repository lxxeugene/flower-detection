...
const SearchResult = (props) => {
  ...
  useEffect(() => {
    const fetchFlowerInfo = async () => {
      try {
        ...
        const response = await axios.get(`https://fdy-5349cb70f365.herokuapp.com/flowers?flowername=${label}`);
  ...
  return (
    <div>
      {flowerInfo && (
      <div style={{ backgroundColor: 'white', padding: '10px', borderRadius: '8px', display: 'flex', flexDirection: 'column'}}>
      ...
      <span style={{ borderBottom: `3px solid darkgreen`, paddingBottom: '2px'}}>
       ...
       <table style={{ borderSpacing: '10px', wordBreak: 'keep-all'}}>  
         <tbody>
            ...
            <tr>
             <td style={{ color: 'black', width: '15%' }}><strong>판매 검색</strong></td>
             <td style={{ color: 'black', width: '85%' }}>
              <button onClick={goShoppingPage} style={{backgroundColor: "#03C75A", display:'flex', justifyContent: "center", textAlign: "center", border: 'none' }} >
                  <img src={naver} style={{marginRight: "5px"}} width="20" height="20" alt="네이버 로고" />
                  네이버 쇼핑
              </button>
             </td>
            </tr>
